document.addEventListener("DOMContentLoaded", function () {
      ...
      <div style="margin-top: 2rem;">
        ...
        <div style="background: var(--card-bg); padding: 1.5rem; border-radius: 12px; margin: 1rem 0;">
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem;">
            <div style="text-align: center; padding: 1rem; background: white; border-radius: 8px;">
              ...
              <div style="color: var(--main-color); margin-top: 0.5rem;">🏋️ Тренування (70%)</div>
            </div>
            <div style="text-align: center; padding: 1rem; background: white; border-radius: 8px;">
              <div style="font-size: 1.5rem; font-weight: bold; color: #28a745;">${formatNumber(valSamples)}</div>
              <div style="color: var(--main-color); margin-top: 0.5rem;">✅ Валідація (15%)</div>
            </div>
            <div style="text-align: center; padding: 1rem; background: white; border-radius: 8px;">
              <div style="font-size: 1.5rem; font-weight: bold; color: #ffc107;">${formatNumber(testSamples)}</div>
              <div style="color: var(--main-color); margin-top: 0.5rem;">🧪 Тестування (15%)</div>
            </div>
          </div>
        </div>
        ...
        <div style="background: var(--card-bg); padding: 1.5rem; border-radius: 12px; margin: 1rem 0;">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem; margin-top: 1rem;">
            <div><strong>Потреби в зберіганні:</strong> ${storageNeeds}</div>
            <div><strong>Мінімальний датасет:</strong> ${formatNumber(Math.ceil(totalSamples * 0.3))}</div>
            <div><strong>Оптимальний розмір:</strong> ${formatNumber(totalSamples)}</div>
            <div><strong>Максимальна користь:</strong> ${formatNumber(Math.ceil(totalSamples * 2))}</div>
          </div>
        </div>

        <div style="background: var(--card-bg); padding: 1.5rem; border-radius: 12px; margin: 1rem 0;">
          <h5>🎯 Статус проекту: <span style="color: ${getStatusColor(efficiencyStatus.status)};">${efficiencyStatus.text}</span></h5>
  ...
  function calculateCollectionTime(samples, taskType) {
    ...
    if (totalHours < 24) {
      return { value: Math.ceil(totalHours), unit: 'годин' };
    } else if (totalHours < 168) {
      return { value: Math.ceil(totalHours / 24), unit: 'днів' };
    } else if (totalHours < 720) {
      return { value: Math.ceil(totalHours / 168), unit: 'тижнів' };
    } else {
      return { value: Math.ceil(totalHours / 720), unit: 'місяців' };
    }
  }
  ...
  function calculateStorageNeeds(samples, taskType) {
    ...
    if (totalMB < 1024) {
      return `${Math.ceil(totalMB)} МБ`;
    } else if (totalMB < 1024 * 1024) {
      return `${(totalMB / 1024).toFixed(1)} ГБ`;
    } else {
      return `${(totalMB / (1024 * 1024)).toFixed(1)} ТБ`;
    }
  }
  ...
  function getEfficiencyStatus(totalSamples, taskType) {
    ...
    if (totalSamples < threshold.low) {
      return {
        status: 'warning',
        text: 'Малий датасет',
        description: 'Датасет може бути занадто малим для досягнення цільової точності. Розгляньте використання transfer learning або data augmentation.'
      };
    } else if (totalSamples > threshold.high) {
      return {
        status: 'info',
        text: 'Великий датасет',
        description: 'Великий датасет забезпечить високу точність, але потребує значних ресурсів. Переконайтеся, що у вас є достатньо часу та бюджету.'
      };
    } else {
      return {
        status: 'success',
        text: 'Оптимальний розмір',
        description: 'Розмір датасету оптимально збалансований для досягнення цільової точності з розумними витратами ресурсів.'
      };
    }
  }
  ...
  function generateRecommendations(totalSamples, taskType, transferLearning, dataAugmentation) {
    ...
    if (totalSamples > 100000) {
      recommendations.push('<li>📚 Великий датасет - розгляньте поетапний збір та навчання</li>');
    }
    
    if (transferLearning === 'none') {
      recommendations.push('<li>🚀 Використайте transfer learning для значного зменшення потрібного датасету</li>');
    }
    
    if (dataAugmentation === 'none') {
      recommendations.push('<li>🔄 Data augmentation може зменшити потребу в реальних даних у 2-3 рази</li>');
    }
    
    if (taskType === 'image_classification') {
      recommendations.push('<li>🖼️ Для зображень рекомендуємо ImageNet pre-trained моделі</li>');
    }
    
    if (taskType === 'nlp_classification') {
      recommendations.push('<li>📝 Використайте BERT або подібні попередньо навчені мовні моделі</li>');
    }
    
    recommendations.push('<li>🎯 Почніть з меншого датасету та поступово збільшуйте розмір</li>');
    recommendations.push('<li>📊 Регулярно перевіряйте метрики на валідаційному наборі</li>');
    recommendations.push('<li>🔍 Приділіть особливу увагу якості розмітки - це критично важливо</li>');
