document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateOptionsDisplay() {
    if (options.length === 0) {
      optionsList.innerHTML = '<p class="empty-state">Почніть додавати варіанти для колеса</p>';
      return;
    }
  ...
  function spinWheel() {
    ...
    spinBtn.querySelector('.spin-text').textContent = "Крутиться...";
    ...
    const animate = () => {
      ...
      if (progress < 1) {
        ...
        spinBtn.querySelector('.spin-text').textContent = "Крутити";
  ...
  function showResult(winner, index) {
    result.innerHTML = `
      ...
      <small>${eliminateOption.checked && options.length > 1 ? 
        'Варіант буде видалений з колеса' : 
