document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (!cpuScore || !gpuScore) {
      result.innerHTML = `<div class="insight-card warning">
        <h6>⚠️ Компонент не знайдено</h6>
        <p>Не вдалося знайти один або обидва компоненти в базі даних. Спробуйте ввести точнішу назву моделі.</p>
        <p><small>Приклад: "Intel i5-12400F" або "RTX 4060"</small></p>
      </div>`;
      return;
    }
    ...
    result.innerHTML = `
      <div class="insight-cards">
        <div class="insight-card ${analysis.severity.class}">
          <h6>${analysis.severity.icon} Вузьке місце</h6>
        ...
        <div class="insight-card info">
          ...
          <p>Продуктивність процесора</p>
        </div>
        
        <div class="insight-card info">
          <h6>🎮 GPU Рейтинг</h6>
          <div class="big-number">${gpuScore}</div>
          <p>Продуктивність відеокарти</p>
        </div>
      </div>
        ...
        <div style="text-align: left;">
          <div class="component-analysis">
            ...
            <div class="component-bar">
              ...
              <span>${gpuScore} балів</span>
            </div>
          </div>
          
          <div style="margin-top: 1rem;">
            <p><strong>💾 RAM:</strong> ${ramAmount} GB ${analysis.ramStatus}</p>
            <p><strong>🖥️ Роздільність:</strong> ${getResolutionName(resolution)}</p>
            <p><strong>🎯 Призначення:</strong> ${getUseCaseName(useCase)}</p>
            <p><strong>⚖️ Баланс системи:</strong> ${analysis.balanceDescription}</p>
          </div>
        </div>
      </div>
      ...
      <div class="insight-card">
        <h6>🎯 Прогноз продуктивності</h6>
        ${generatePerformanceForecast(analysis, useCase, targetFps, resolution, settingsLevel)}
      </div>

      <div class="insight-card info">
        <h6>💡 Загальні поради</h6>
        <ul style="text-align: left; margin: 1rem 0;">
          <li>📊 Використовуйте MSI Afterburner для моніторингу завантаження компонентів</li>
          <li>🎛️ Експериментуйте з налаштуваннями для оптимізації балансу</li>
          <li>❄️ Переконайтеся в адекватному охолодженні системи</li>
          <li>⚡ Перевірте, чи достатньо потужності блоку живлення</li>
          <li>🔧 Розгляньте апгрейд найслабшого компонента першим</li>
          <li>💾 Переконайтеся, що RAM працює на заявленій частоті</li>
        </ul>
      </div>
    `;
  ...
  function calculateBottleneck(cpuScore, gpuScore, ramAmount, resolution, useCase, settingsLevel, targetFps) {
    ...
    if (adjustedCpuScore < adjustedGpuScore) {
      ...
      bottleneckType = "Збалансована система";
    }
    ...
    if (ramAmount < requiredRam[useCase]) {
      ramStatus = "(⚠️ Недостатньо для задач)";
    } else {
      ramStatus = "(✅ Достатньо)";
    }
    ...
    if (bottleneckPercent <= 10) {
      severity = { class: "success", icon: "🟢", level: "Відмінно" };
    } else if (bottleneckPercent <= 20) {
      severity = { class: "success", icon: "🟡", level: "Добре" };
    } else if (bottleneckPercent <= 30) {
      severity = { class: "warning", icon: "🟠", level: "Помірно" };
    } else {
      severity = { class: "warning", icon: "🔴", level: "Критично" };
    }

    let balanceDescription;
    if (bottleneckPercent <= 5) {
      balanceDescription = "Ідеально збалансована система";
    } else if (bottleneckPercent <= 15) {
      balanceDescription = "Добре збалансована система";
    } else if (bottleneckPercent <= 25) {
      balanceDescription = "Помірний дисбаланс компонентів";
    } else {
      balanceDescription = "Значний дисбаланс компонентів";
    }
  ...
  function generateRecommendations(analysis, useCase, resolution, settingsLevel) {
    ...
    if (analysis.bottleneckType === "CPU Bottleneck") {
      recommendations += "<li><strong>🔥 CPU Bottleneck виявлено:</strong><br>";
      recommendations += "• Знизьте налаштування, що навантажують CPU (фізика, NPC, дальність прорисовки)<br>";
      recommendations += "• Підвищте роздільність екрану або графічні налаштування<br>";
      recommendations += "• Розгляньте апгрейд процесора</li>";
    } else if (analysis.bottleneckType === "GPU Bottleneck") {
      recommendations += "<li><strong>🎮 GPU Bottleneck виявлено:</strong><br>";
      recommendations += "• Знизьте графічні налаштування (текстури, тіні, анти-аліазинг)<br>";
      recommendations += "• Зменште роздільність екрану<br>";
      recommendations += "• Розгляньте апгрейд відеокарти</li>";
    } else {
      recommendations += "<li><strong>⚖️ Система добре збалансована:</strong><br>";
      recommendations += "• Ваша конфігурація працює оптимально<br>";
      recommendations += "• При потребі можна підвищити налаштування графіки</li>";
    }

    // RAM recommendations
    if (analysis.ramStatus.includes("⚠️")) {
      recommendations += "<li><strong>💾 Рекомендації щодо RAM:</strong><br>";
      recommendations += "• Збільште обсяг оперативної пам'яті<br>";
      recommendations += "• Переконайтеся в роботі RAM у двоканальному режимі</li>";
    }

    // Use case specific recommendations
    if (useCase === "gaming" && analysis.bottleneckPercent > 20) {
      recommendations += "<li><strong>🎮 Для ігор:</strong><br>";
      recommendations += "• Експериментуйте з налаштуваннями V-Sync та FPS лімітів<br>";
      recommendations += "• Використовуйте технології DLSS/FSR якщо доступні</li>";
    }

    if (useCase === "content-creation" && analysis.bottleneckType === "CPU Bottleneck") {
      recommendations += "<li><strong>🎬 Для контент-креації:</strong><br>";
      recommendations += "• Використовуйте апаратне прискорення рендерингу<br>";
      recommendations += "• Розгляньте процесори з більшою кількістю ядер</li>";
    }
  ...
  function generatePerformanceForecast(analysis, useCase, targetFps, resolution, settingsLevel) {
    ...
    forecast += `<strong>📈 Очікувана продуктивність:</strong> ${performanceLevel}<br>`;
    forecast += `<strong>🎯 Прогноз FPS:</strong> ~${fpsEstimate} FPS (ціль: ${targetFps} FPS)<br>`;
    forecast += `<strong>🎮 Для ${getUseCaseName(useCase)}:</strong> `;
    
    if (analysis.bottleneckPercent <= 15) {
      forecast += "Система справиться чудово ✅";
    } else if (analysis.bottleneckPercent <= 25) {
      forecast += "Потрібні компроміси в налаштуваннях ⚠️";
    } else {
      forecast += "Рекомендується апгрейд 🔴";
    }
    forecast += `</div>`;
  ...
  function getUseCaseName(useCase) {
    const names = {
      "gaming": "Ігри",
      "content-creation": "Контент-креація",
      "3d-modeling": "3D моделювання",
      "office-work": "Офісна робота",
      "programming": "Програмування",
      "mixed": "Змішане використання"
    };
