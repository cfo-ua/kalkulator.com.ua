  ...
  form.addEventListener("submit", function (e) {
    ...
    if (inputType === 'circumference') {
      ...
      if (isNaN(circumference) || circumference <= 0) {
        result.innerHTML = "<p>Будь ласка, введіть коректний обхват пальця.</p>";
        return;
      }
      ...
      if (isNaN(diameter) || diameter <= 0) {
        result.innerHTML = "<p>Будь ласка, введіть коректний діаметр пальця.</p>";
        return;
      }
      ...
      if (!knownSize) {
        result.innerHTML = "<p>Будь ласка, введіть відомий розмір кільця.</p>";
        return;
      }
      diameter = convertKnownSizeToDiameter(system, knownSize);
      if (!diameter) {
        result.innerHTML = "<p>Не вдалося розпізнати введений розмір. Перевірте правильність введення.</p>";
        return;
      }
    }
    ...
    if (ringSize) {
      ...
      result.innerHTML = `
        <div class="insight-cards">
          <div class="insight-card success">
            <h6>💍 Діаметр кільця</h6>
            <div class="big-number">${diameter.toFixed(1)} мм</div>
            <p>Внутрішній діаметр</p>
          </div>
          
          <div class="insight-card info">
            <h6>📏 Обхват пальця</h6>
            <div class="big-number">${circumference.toFixed(1)} мм</div>
            <p>Довжина навколо пальця</p>
          </div>
        </div>
        
        <div style="margin-top: 2rem;">
          <h3>🌍 Міжнародні розміри кілець</h3>
          
          <table style="width: 100%; margin-bottom: 1.5rem; border-collapse: collapse;">
            <tr>
              <th style="padding: 12px; border: 1px solid var(--border); background: var(--card-bg);">Система</th>
              <th style="padding: 12px; border: 1px solid var(--border); background: var(--card-bg);">Розмір</th>
              <th style="padding: 12px; border: 1px solid var(--border); background: var(--card-bg);">Опис</th>
            </tr>
            <tr>
              <td style="padding: 12px; border: 1px solid var(--border);"><strong>🇺🇦 Українська (UA)</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border); font-size: 1.2em; color: var(--accent);"><strong>${ringSize.ua}</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border);">Діаметр в міліметрах</td>
            </tr>
            <tr>
              <td style="padding: 12px; border: 1px solid var(--border);"><strong>🇪🇺 Європейська (EU)</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border); font-size: 1.2em; color: var(--accent);"><strong>${ringSize.eu}</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border);">Аналогічно українській</td>
            </tr>
            <tr>
              <td style="padding: 12px; border: 1px solid var(--border);"><strong>🇺🇸 Американська (US)</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border); font-size: 1.2em; color: var(--accent);"><strong>${ringSize.us}</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border);">Цифрове позначення</td>
            </tr>
            <tr>
              <td style="padding: 12px; border: 1px solid var(--border);"><strong>🇬🇧 Британська (UK)</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border); font-size: 1.2em; color: var(--accent);"><strong>${ringSize.uk}</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border);">Літерне позначення</td>
            </tr>
            <tr>
              <td style="padding: 12px; border: 1px solid var(--border);"><strong>🇯🇵 Японська (JP)</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border); font-size: 1.2em; color: var(--accent);"><strong>${ringSize.jp}</strong></td>
              <td style="padding: 12px; border: 1px solid var(--border);">Подвійні цифри</td>
            </tr>
          </table>
          
          <div style="background: #f0f8ff; padding: 1.5rem; border-radius: 12px; margin-top: 1.5rem; border-left: 4px solid var(--accent);">
            <h4>💡 Поради щодо вибору кільця:</h4>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem; margin-top: 1rem;">
              <div style="background: white; padding: 1rem; border-radius: 8px;">
                <h5>📏 Вузькі кільця (до 4мм)</h5>
                <p><strong>Розмір: ${ringSize.ua}</strong></p>
                <p>Точно за вашим розміром</p>
              </div>
              <div style="background: white; padding: 1rem; border-radius: 8px;">
                <h5>📏 Широкі кільця (понад 4мм)</h5>
                <p><strong>Розмір: ${getWideRingSize(ringSize.ua)}</strong></p>
                <p>На 0.5-1 розмір більше</p>
              </div>
              <div style="background: white; padding: 1rem; border-radius: 8px;">
                <h5>💒 Обручки</h5>
                <p><strong>Розмір: ${getWeddingRingSize(ringSize.ua)}</strong></p>
                <p>Трохи більше для комфорту</p>
              </div>
            </div>
          </div>
          
          <div style="background: var(--card-bg); padding: 1.5rem; border-radius: 12px; margin-top: 1.5rem;">
            <h4>📐 Ваші параметри:</h4>
            <ul style="list-style: none; padding: 0;">
              <li>🔸 Діаметр пальця: <strong>${diameter.toFixed(1)} мм</strong></li>
              <li>🔸 Обхват пальця: <strong>${circumference.toFixed(1)} мм</strong></li>
              <li>🔸 Український розмір: <strong>${ringSize.ua}</strong></li>
            </ul>
          </div>
          
          <div style="background: #fff8e1; padding: 1.5rem; border-radius: 12px; margin-top: 1.5rem; border-left: 4px solid #ffc107;">
            <h4>⚠️ Важливі нагадування:</h4>
            <ul>
              <li><strong>Час вимірювання:</strong> Краще вимірювати ввечері при нормальній температурі</li>
              <li><strong>Ширина кільця:</strong> Широкі кільця потребують більшого розміру</li>
              <li><strong>Сустави:</strong> Кільце має проходити через сустав, але щільно сидіти</li>
              <li><strong>Професійна допомога:</strong> У разі сумнівів зверніться до ювеліра</li>
              <li><strong>Зміна розміру:</strong> Більшість кілець можна підігнати у ювеліра</li>
            </ul>
          </div>
        </div>
      `;
      ...
      result.innerHTML = `
        <div class="insight-card warning">
          <h6>⚠️ Нестандартний розмір</h6>
          <p>Введений розмір виходить за межі стандартних розмірів кілець. Рекомендуємо:</p>
          <ul style="text-align: left; margin-top: 1rem;">
            <li>Перевірити правильність вимірювань</li>
            <li>Звернутися до ювеліра для індивідуального виготовлення</li>
            <li>Розглянути регульовані моделі кілець</li>
          </ul>
        </div>
      `;
    }
