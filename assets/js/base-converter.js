  ...
  function convertFromInput(sourceInput) {
    try {
      ...
      switch (sourceInput.id) {
        ...
        case 'bc-binary':
          if (!/^[01]+$/.test(value)) {
            throw new Error('Невірний двійковий формат');
          }
          decimalValue = parseInt(value, 2);
          break;
        case 'bc-octal':
          if (!/^[0-7]+$/.test(value)) {
            throw new Error('Невірний вісімковий формат');
          }
          decimalValue = parseInt(value, 8);
          break;
        case 'bc-hex':
          if (!/^[0-9A-Fa-f]+$/.test(value)) {
            throw new Error('Невірний шістнадцятковий формат');
          }
          decimalValue = parseInt(value, 16);
          break;
        default:
          throw new Error('Невідоме джерело');
      }
      ...
      if (isSigned) {
        if (decimalValue > maxSigned || decimalValue < minSigned) {
          throw new Error(`Число поза межами для ${bitWidth}-бітного знакового числа (${minSigned} до ${maxSigned})`);
        }
      } else {
        if (decimalValue > maxUnsigned || decimalValue < 0) {
          throw new Error(`Число поза межами для ${bitWidth}-бітного беззнакового числа (0 до ${maxUnsigned})`);
        }
      }
  ...
  function displayResults(decimal, binary, octal, hex, isSigned, useTwosComplement, bitWidth) {
    ...
    resultHTML += `
      ...
      <div class="insight-card info">
        ...
        <div class="small-text">${binary.length} біт</div>
      </div>
    ...
    // Bit analysis
    resultHTML += `<h4>🔍 Детальний аналіз</h4>`;
    resultHTML += '<div class="bit-analysis">';
    
    if (isSigned && useTwosComplement && decimal < 0) {
      resultHTML += `<p><strong>Доповнення до двох:</strong> ${binary}</p>`;
      resultHTML += `<p><strong>Оригінальне від'ємне значення:</strong> ${decimal}</p>`;
    }
    
    // Power breakdown for decimal
    if (binary.length <= 16) { // Only for reasonable lengths
      resultHTML += '<p><strong>Розклад за степенями:</strong><br>';
      const powers = [];
