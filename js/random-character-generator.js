document.addEventListener('DOMContentLoaded', function() {
    ...
    let history = JSON.parse(localStorage.getItem('charHistory_ua') || '[]');
    ...
    function generateCharacters() {
        ...
        if (length < 1 || length > 1000) {
            alert('Довжина повинна бути від 1 до 1000 символів');
            return;
        }
        
        if (count < 1 || count > 50) {
            alert('Кількість повинна бути від 1 до 50');
            return;
        }
        ...
        if (!charset) {
            alert('Оберіть принаймні один тип символів або введіть власний набір');
            return;
        }
        
        if (!allowRepeats && length > charset.length) {
            alert(`Неможливо створити ${length} унікальних символів з набору ${charset.length} символів`);
            return;
        }
    ...
    function displayResults(sequences, timestamp) {
        const sequencesList = sequences.map((item, index) => 
            `<div class="code-item">
                ...
                <span class="code-format">${item.length} символів</span>
        ...
        generationInfo.innerHTML = `
            <div class="info-row">
                <span>📊 Згенеровано: <strong>${sequences.length}</strong> послідовностей</span>
                <span>🎭 Набір: <strong>${charsetSize}</strong> символів</span>
    ...
    function copyToClipboard(text) {
        navigator.clipboard.writeText(text).then(() => {
            showNotification('Скопійовано: ' + text.substring(0, 20) + (text.length > 20 ? '...' : ''));
        }).catch(() => {
    ...
    function copyAllCharacters() {
        ...
        if (charElements.length === 0) {
            alert('Спочатку згенеруйте символи');
            return;
        }
    ...
    function addToHistory(sequences, timestamp, length) {
        ...
        localStorage.setItem('charHistory_ua', JSON.stringify(history));
        updateHistoryDisplay();
    }
        ...
        const historyHTML = history.map((entry, index) => `
            <div class="history-entry">
                <div class="history-header">
                    <span class="entry-info">${entry.count} × ${entry.length} символів</span>
                    <span class="entry-time">${entry.timestamp}</span>
                </div>
                <div class="history-codes">
                    ${entry.sequences.slice(0, 3).map(item => 
                        `<span class="history-code" onclick="copyToClipboard('${item.sequence}')" title="Клікніть для копіювання">${item.sequence.substring(0, 15)}${item.sequence.length > 15 ? '...' : ''}</span>`
                    ).join('')}
                    ${entry.sequences.length > 3 ? `<span class="more-codes">+${entry.sequences.length - 3} ще</span>` : ''}
                </div>
            </div>
        `).join('');
    ...
    function clearHistory() {
        if (confirm('Ви впевнені, що хочете очистити історію?')) {
            history = [];
            localStorage.removeItem('charHistory_ua');
            updateHistoryDisplay();
