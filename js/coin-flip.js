document.addEventListener("DOMContentLoaded", () => {
  ...
  function flipCoin() {
    ...
    flipBtn.querySelector('.button-text').textContent = "Підкидаю...";
  ...
  function loadStats() {
    try {
      const savedStats = localStorage.getItem('coinFlipStatsUA');
      if (savedStats) {
      ...
      console.log('Не вдалося завантажити статистику:', error);
    }
  }
  
  function saveStats() {
    try {
      localStorage.setItem('coinFlipStatsUA', JSON.stringify(stats));
    } catch (error) {
      console.log('Не вдалося зберегти статистику:', error);
    }
  }
