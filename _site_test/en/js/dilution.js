document.addEventListener("DOMContentLoaded", function () {
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (filled !== 3) {
        result.textContent = "Enter exactly three parameters.";
        return;
      }
      if (isNaN(c1)) {
        result.innerHTML = `<b>C₁ =</b> ${(c2 * v2 / v1).toFixed(4)} mol/L`;
      } else if (isNaN(v1)) {
        result.innerHTML = `<b>V₁ =</b> ${(c2 * v2 / c1).toFixed(4)} L`;
      } else if (isNaN(c2)) {
        result.innerHTML = `<b>C₂ =</b> ${(c1 * v1 / v2).toFixed(4)} mol/L`;
      } else if (isNaN(v2)) {
        result.innerHTML = `<b>V₂ =</b> ${(c1 * v1 / c2).toFixed(4)} L`;
      }
    });
  }
});