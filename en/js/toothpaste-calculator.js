document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (totalUsers !== usersCount) {
      result.innerHTML = '<p style="color:red;">Number of people doesn\'t match age group breakdown.</p>';
      return;
    }
    ...
    if (weightedDailyUsage > 3) {
      recommendations.push("📏 Try using less paste - a rice grain-sized amount is sufficient");
    }
    if (squeezingMethod === 'random' || squeezingMethod === 'middle') {
      recommendations.push("💡 Squeeze paste from the end of tube for savings");
    }
    if (storage === 'careless') {
      recommendations.push("🔒 Always close tube after use");
    }
    if (pasteAmount > 1.0) {
      recommendations.push("⚖️ Reduce paste amount - large portions don't improve cleaning");
    }
    if (costPerYear > 60) {
      recommendations.push("💰 Consider more economical brand or buy in bulk");
    }
    if (sharing === 'family' && totalUsers > 3) {
      recommendations.push("👨‍👩‍👧‍👦 For large families, bigger tubes are more economical");
    }

    // Generate tips
    let tips = [];
    tips.push("🦷 Cleaning quality depends on technique, not amount of paste");
    tips.push("⏰ Brush teeth for minimum 2 minutes for effective plaque removal");
    tips.push("🔄 Replace toothbrush every 3-4 months");
    if (specialNeeds !== 'none') {
      tips.push("👨‍⚕️ Consult dentist about special needs");
    }

    const resultHTML = `
      <div style="background:#f8f9fa;padding:20px;border-radius:8px;margin:20px 0;">
        <h3 style="color:#157aff;margin-top:0;">Toothpaste Consumption Analysis</h3>
        ...
        <div class="insight-card">
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:15px;margin-bottom:20px;">
            <div style="text-align:center;background:#e3f2fd;padding:15px;border-radius:8px;">
              ...
              <p style="margin:5px 0;font-size:0.9em;color:#666;">Tube duration</p>
            </div>
            <div style="text-align:center;background:#e8f5e8;padding:15px;border-radius:8px;">
              ...
              <p style="margin:5px 0;font-size:0.9em;color:#666;">Cost per day</p>
            </div>
            <div style="text-align:center;background:#fff3e0;padding:15px;border-radius:8px;">
              <div style="font-size:1.5em;font-weight:bold;color:#f57c00;">
                ${weightedDailyUsage.toFixed(1)} g
              </div>
              <p style="margin:5px 0;font-size:0.9em;color:#666;">Daily consumption</p>
            </div>
          </div>
        </div>

        <div class="insight-card">
          <h4 style="margin-top:0;color:#28a745;">📅 Consumption Details</h4>
          <div style="background:white;padding:15px;border-radius:6px;">
            <table style="width:100%;border-collapse:collapse;">
              <thead>
                <tr style="background:#f8f9fa;">
                  <th style="padding:10px;text-align:left;border-bottom:2px solid #dee2e6;">Metric</th>
                  <th style="padding:10px;text-align:center;border-bottom:2px solid #dee2e6;">Value</th>
                  <th style="padding:10px;text-align:center;border-bottom:2px solid #dee2e6;">Cost</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td style="padding:10px;border-bottom:1px solid #dee2e6;">Brushings per tube</td>
                  <td style="padding:10px;text-align:center;border-bottom:1px solid #dee2e6;">${Math.round(brushesPerTube)}</td>
                  <td style="padding:10px;text-align:center;border-bottom:1px solid #dee2e6;">${costPerBrush.toFixed(3)}/brush</td>
                </tr>
                <tr>
                  <td style="padding:10px;border-bottom:1px solid #dee2e6;">Per person consumption</td>
                  <td style="padding:10px;text-align:center;border-bottom:1px solid #dee2e6;">${gramsPerPerson.toFixed(2)} g/day</td>
                  <td style="padding:10px;text-align:center;border-bottom:1px solid #dee2e6;">${formatMoney(costPerPerson)}/year</td>
                </tr>
                <tr>
                  <td style="padding:10px;border-bottom:1px solid #dee2e6;">Weekly consumption</td>
                  <td style="padding:10px;text-align:center;border-bottom:1px solid #dee2e6;">${(weightedDailyUsage * 7).toFixed(1)} g</td>
                  <td style="padding:10px;text-align:center;border-bottom:1px solid #dee2e6;">${formatMoney(costPerWeek)}</td>
                </tr>
                <tr style="background:#e8f5e8;">
                  <td style="padding:10px;border-bottom:1px solid #dee2e6;"><strong>Annual consumption</strong></td>
                  <td style="padding:10px;text-align:center;border-bottom:1px solid #dee2e6;"><strong>${tubesPerYear.toFixed(1)} tubes</strong></td>
        ...
        <div class="insight-card">
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:15px;">
            <div style="background:#fff3cd;padding:15px;border-radius:6px;">
              ...
              Per month: ${formatMoney(costPerMonth)}<br>
              Per year: ${formatMoney(annualCost)}<br>
              Tubes per year: ${tubesPerYear.toFixed(1)}
            </div>
            <div style="background:#d1ecf1;padding:15px;border-radius:6px;">
              <strong>Bulk purchases:</strong><br>
              Annual savings: ${formatMoney(annualSavings)}<br>
              Optimized cost: ${formatMoney(bulkAnnualCost)}<br>
              Discount: ${(bulkDiscount * 100).toFixed(0)}%
            </div>
            <div style="background:#f8d7da;padding:15px;border-radius:6px;">
              <strong>Efficiency:</strong><br>
              Grams per person: ${gramsPerPerson.toFixed(2)}<br>
              Tube usage: ${((usableAmount/tubeWeightGrams)*100).toFixed(0)}%<br>
              Waste: ${(((tubeWeightGrams-usableAmount)/tubeWeightGrams)*100).toFixed(0)}%
            </div>
          </div>
        </div>

        <div class="insight-card">
          <h4 style="margin-top:0;color:#6f42c1;">👥 Age Group Breakdown</h4>
          <div style="background:white;padding:15px;border-radius:6px;">
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:15px;">
              ${adults > 0 ? `
              <div style="background:#f8f9fa;padding:10px;border-radius:6px;">
                <strong>Adults (${adults}):</strong><br>
                ${(adults * baseUsagePerBrushing * brushingFrequency * ageFactors.adults).toFixed(1)} g/day
              </div>` : ''}
              ${teenagers > 0 ? `
              <div style="background:#f8f9fa;padding:10px;border-radius:6px;">
                <strong>Teenagers (${teenagers}):</strong><br>
                ${(teenagers * baseUsagePerBrushing * brushingFrequency * ageFactors.teenagers).toFixed(1)} g/day
              </div>` : ''}
              ${children > 0 ? `
              <div style="background:#f8f9fa;padding:10px;border-radius:6px;">
                <strong>Children (${children}):</strong><br>
                ${(children * baseUsagePerBrushing * brushingFrequency * ageFactors.children).toFixed(1)} g/day
              </div>` : ''}
              ${toddlers > 0 ? `
              <div style="background:#f8f9fa;padding:10px;border-radius:6px;">
                <strong>Toddlers (${toddlers}):</strong><br>
                ${(toddlers * baseUsagePerBrushing * brushingFrequency * ageFactors.toddlers).toFixed(1)} g/day
              </div>` : ''}
            </div>
          </div>
        </div>
        ...
        <div class="insight-card">
          <h4 style="margin-top:0;color:#17a2b8;">🧪 Paste Characteristics</h4>
          <div style="background:#d1ecf1;padding:15px;border-radius:6px;">
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:15px;">
              <div style="color:#0c5460;">
                <strong>Paste type:</strong> ${getPasteTypeText(pasteType)}<br>
                <strong>Consistency:</strong> ${getConsistencyText(pasteConsistency)}<br>
                <strong>Density:</strong> ${getDensityText(pasteDensity)}
              </div>
              <div style="color:#0c5460;">
                <strong>Tube volume:</strong> ${tubeVolume} ml<br>
                <strong>Paste weight:</strong> ~${tubeWeightGrams.toFixed(0)} g<br>
                <strong>Usable:</strong> ${usableAmount.toFixed(0)} g
              </div>
              <div style="color:#0c5460;">
                <strong>Brush:</strong> ${getBrushTypeText(brushType)}<br>
                <strong>Technique:</strong> ${getTechniqueText(brushingTechnique)}<br>
                <strong>Frequency:</strong> ${brushingFrequency} times/day
              </div>
            </div>
          </div>
        </div>
        ...
        <div style="background:#e2e3e5;padding:15px;border-radius:6px;margin:15px 0;">
          <h4 style="margin-top:0;color:#383d41;">📋 Dental Health Tips</h4>
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:15px;">
            <div style="color:#383d41;">
              <strong>🦷 Proper brushing:</strong>
            ...
            <div style="color:#383d41;">
              ...
              <ul style="margin:5px 0;font-size:0.9em;">
                <li>Buy large tubes for savings</li>
                <li>Use pea-sized amount of paste</li>
                <li>Squeeze from end of tube</li>
                <li>Cut tube for complete use</li>
              </ul>
            </div>
          </div>
        </div>
