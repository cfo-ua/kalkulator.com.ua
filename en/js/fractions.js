document.addEventListener("DOMContentLoaded", function () {
  ...
  function generateAdditionSteps(frac1, frac2, result) {
    ...
    steps.push(`<div class="step"><strong>Original problem:</strong> ${frac1} + ${frac2}</div>`);
    
    if (frac1.denominator === frac2.denominator) {
      steps.push(`<div class="step">Same denominators, add numerators: ${frac1.numerator} + ${frac2.numerator} = ${frac1.numerator + frac2.numerator}</div>`);
      steps.push(`<div class="step">Result: ${frac1.numerator + frac2.numerator}/${frac1.denominator}</div>`);
    } else {
      ...
      steps.push(`<div class="step">Find common denominator: LCM(${frac1.denominator}, ${frac2.denominator}) = ${commonDenom}</div>`);
      steps.push(`<div class="step">Convert to common denominator:</div>`);
      steps.push(`<div class="step">${frac1} = ${newNum1}/${commonDenom}</div>`);
      steps.push(`<div class="step">${frac2} = ${newNum2}/${commonDenom}</div>`);
      steps.push(`<div class="step">Add numerators: ${newNum1} + ${newNum2} = ${newNum1 + newNum2}</div>`);
      steps.push(`<div class="step">Result: ${newNum1 + newNum2}/${commonDenom}</div>`);
    }
    
    if (result.toString() !== `${result.numerator}/${result.denominator}`) {
      steps.push(`<div class="step">Simplify fraction: ${result}</div>`);
    }
  ...
  function generateSubtractionSteps(frac1, frac2, result) {
    ...
    if (frac1.denominator === frac2.denominator) {
      steps.push(`<div class="step">Same denominators, subtract numerators: ${frac1.numerator} − ${frac2.numerator} = ${frac1.numerator - frac2.numerator}</div>`);
      steps.push(`<div class="step">Result: ${frac1.numerator - frac2.numerator}/${frac1.denominator}</div>`);
    } else {
      const commonDenom = lcm(frac1.denominator, frac2.denominator);
      const newNum1 = frac1.numerator * (commonDenom / frac1.denominator);
      ...
      steps.push(`<div class="step">${frac1} = ${newNum1}/${commonDenom}</div>`);
      steps.push(`<div class="step">${frac2} = ${newNum2}/${commonDenom}</div>`);
      steps.push(`<div class="step">Subtract numerators: ${newNum1} − ${newNum2} = ${newNum1 - newNum2}</div>`);
      steps.push(`<div class="step">Result: ${newNum1 - newNum2}/${commonDenom}</div>`);
    }
  ...
  function generateMultiplicationSteps(frac1, frac2, result) {
    ...
    steps.push(`<div class="step"><strong>Original problem:</strong> ${frac1} × ${frac2}</div>`);
    steps.push(`<div class="step">Multiply numerators: ${frac1.numerator} × ${frac2.numerator} = ${frac1.numerator * frac2.numerator}</div>`);
    steps.push(`<div class="step">Multiply denominators: ${frac1.denominator} × ${frac2.denominator} = ${frac1.denominator * frac2.denominator}</div>`);
    steps.push(`<div class="step">Result: ${frac1.numerator * frac2.numerator}/${frac1.denominator * frac2.denominator}</div>`);
  ...
  function generateDivisionSteps(frac1, frac2, result) {
    ...
    steps.push(`<div class="step"><strong>Original problem:</strong> ${frac1} ÷ ${frac2}</div>`);
    steps.push(`<div class="step">Division by fraction = multiplication by reciprocal</div>`);
    steps.push(`<div class="step">Reciprocal of ${frac2}: ${frac2.denominator}/${frac2.numerator}</div>`);
    steps.push(`<div class="step">Multiply: ${frac1} × ${frac2.denominator}/${frac2.numerator}</div>`);
    steps.push(`<div class="step">Numerator: ${frac1.numerator} × ${frac2.denominator} = ${frac1.numerator * frac2.denominator}</div>`);
    steps.push(`<div class="step">Denominator: ${frac1.denominator} × ${frac2.numerator} = ${frac1.denominator * frac2.numerator}</div>`);
    steps.push(`<div class="step">Result: ${frac1.numerator * frac2.denominator}/${frac1.denominator * frac2.numerator}</div>`);
  ...
  function displayResults(frac1, frac2, result, operationName, steps) {
    ...
    if (result.isImproper()) {
      simplifiedResult.innerHTML += `<br><small>Mixed number: ${result.toMixed()}</small>`;
    }
