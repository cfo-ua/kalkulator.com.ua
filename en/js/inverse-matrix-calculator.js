    ...
    if (Math.abs(det) < 1e-10) {
      result.innerHTML = `
        <div class="insight-card warning">
          <h6>⚠️ Singular Matrix</h6>
          <div class="result-value">Determinant = ${det.toFixed(8)}</div>
          <p>Inverse matrix does not exist because the determinant equals zero.</p>
        </div>
      `;
      return;
    }
    ...
    result.innerHTML = `
      <div class="insight-cards">
        ...
        <div class="insight-card info">
          <h6>✅ Status</h6>
          <div class="result-value">Matrix is non-singular</div>
          <div class="result-value">Inverse matrix exists</div>
        </div>
      </div>
      
      <div style="margin-top: 2rem;">
        <h4>🔄 Inverse Matrix A⁻¹:</h4>
        ${formatMatrix(inverse)}
      </div>
      
      <div style="margin-top: 1rem; padding: 1rem; background: #f8f9fa; border-radius: 8px;">
        <strong>Verification:</strong> A × A⁻¹ = I (identity matrix)
      </div>
    `;
  });
