document.addEventListener('DOMContentLoaded', function() {
    ...
    let history = JSON.parse(localStorage.getItem('dateHistory_en') || '[]');
    ...
    function formatDate(date, format, includeTime = false) {
        ...
        switch (format) {
            ...
            case 'yyyy-mm-dd':
                ...
                break;
            case 'full-text':
                formattedDate = `${monthNames[date.getMonth()]} ${parseInt(day)}, ${year}`;
                break;
            case 'with-time':
    ...
    function generateDates() {
        ...
        if (count < 1 || count > 100) {
            alert('Number of dates must be between 1 and 100');
            return;
        }
        ...
        if (dates.length === 0) {
            alert('Could not generate dates with selected filters. Try expanding the range.');
            return;
        }
    ...
    function displayResults(dates, timestamp) {
        ...
        generationInfo.innerHTML = `
            <div class="info-row">
                <span>📊 Generated: <strong>${dates.length}</strong> dates</span>
    ...
    function addToHistory(dates, timestamp, format) {
        ...
        localStorage.setItem('dateHistory_en', JSON.stringify(history));
        updateHistoryDisplay();
    }
        ...
        const historyHTML = history.map((entry, index) => `
            <div class="history-entry">
                <div class="history-header">
                    <span class="entry-info">${entry.count} dates (${entry.format})</span>
                    <span class="entry-time">${entry.timestamp}</span>
                </div>
                <div class="history-codes">
                    ${entry.dates.slice(0, 5).map(item => 
                        `<span class="history-code" onclick="copyToClipboard('${item.formatted}')" title="Click to copy">${item.formatted}</span>`
                    ).join('')}
                    ${entry.dates.length > 5 ? `<span class="more-codes">+${entry.dates.length - 5} more</span>` : ''}
                </div>
            </div>
        `).join('');
    ...
    function clearHistory() {
        if (confirm('Are you sure you want to clear the history?')) {
            history = [];
            localStorage.removeItem('dateHistory_en');
            updateHistoryDisplay();
