document.addEventListener("DOMContentLoaded", function () {
  ...
  function parseSubstances(part) {
    ...
    return substances.map(substance => {
      ...
      if (!match) {
        throw new Error(`Invalid substance format: ${substance}`);
      }
  ...
  function updateSubstanceDropdown() {
    ...
    if (currentReaction.reactants.length > 0) {
      ...
      reactantGroup.label = 'Reactants';
      currentReaction.reactants.forEach(substance => {
        const option = document.createElement('option');
        option.value = substance;
        option.textContent = `${substance} (coeff. ${currentReaction.coefficients[substance]})`;
  ...
  function calculateStoichiometry() {
    try {
      ...
      if (!molarMass && (amountType === 'mass' || amountType === 'volume')) {
        ...
        if (!molarMass) {
          throw new Error('Enter molar mass for selected substance');
        }
  ...
  function displayResults(givenSubstance, givenAmount, givenType, results) {
    let resultHTML = `<h4>📊 Stoichiometric Calculation Results</h4>`;
    ...
    Object.entries(results).forEach(([substance, data]) => {
      ...
      resultHTML += `<div class="insight-card ${cardClass}">
        <h6>${substance} ${isGiven ? '(given)' : ''}</h6>
        <div class="substance-results">
          <div><strong>Moles:</strong> ${data.moles.toFixed(3)} mol</div>
          ${data.mass ? `<div><strong>Mass:</strong> ${data.mass.toFixed(2)} g</div>` : ''}
          <div><strong>Volume (STP):</strong> ${data.volume.toFixed(2)} L</div>
          <div class="small-text">Coefficient: ${data.coefficient}</div>
        </div>
      </div>`;
    });
    ...
    resultHTML += `<div class="calculation-explanation">
      <h5>🧮 Calculation Explanation</h5>
      <p>Based on stoichiometric coefficients in equation <strong>${equationInput.value}</strong>, 
      the relationship between substances is determined by their coefficients.</p>
