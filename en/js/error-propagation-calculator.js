document.addEventListener("DOMContentLoaded", function () {
  ...
  function calculateBasicOperation() {
    ...
    if (operation === 'power') {
      ...
      if (operation === 'add') {
        ...
        if (valueB === 0) throw new Error("Division by zero is undefined");
        resultValue = valueA / valueB;
        const relativeErrorA = errorA / Math.abs(valueA);
        const relativeErrorB = errorB / Math.abs(valueB);
        const relativeError = Math.sqrt(relativeErrorA * relativeErrorA + relativeErrorB * relativeErrorB);
  ...
  function calculateFunctionOperation() {
    ...
    switch (func) {
      case 'sqrt':
        if (valueA < 0) throw new Error("Square root of negative number");
      ...
      case 'ln':
        if (valueA <= 0) throw new Error("Logarithm of non-positive number");
  ...
  function displayResult(value, error, formula, calculation, operation) {
    ...
    html += '<h3>📊 Error Propagation Result</h3>';
    
    // Main result
    html += '<div class="result-main">';
    html += `<h4>🎯 Result: <span class="result-value">${value.toFixed(6)} ± ${error.toFixed(6)}</span></h4>`;
    html += `<p>📈 Relative Error: <strong>${relativeError.toFixed(3)}%</strong></p>`;
    html += '</div>';
    ...
    if (relativeError < 1) {
      html += '<p>✅ <strong>High Precision:</strong> Relative error less than 1%</p>';
    } else if (relativeError < 5) {
      html += '<p>🟡 <strong>Good Precision:</strong> Relative error less than 5%</p>';
    } else if (relativeError < 10) {
      html += '<p>🟠 <strong>Acceptable Precision:</strong> Relative error less than 10%</p>';
    } else {
      html += '<p>🔴 <strong>Low Precision:</strong> Relative error greater than 10%</p>';
    }
    
    // Scientific notation
    const scientificValue = value.toExponential(3);
    const scientificError = error.toExponential(2);
    html += `<p>🔬 <strong>Scientific Notation:</strong> (${scientificValue} ± ${scientificError})</p>`;
    ...
    html += `<p>📝 <strong>Recommended Format:</strong> ${roundedValue} ± ${roundedError}</p>`;
    
    // Additional tips
    html += '<div class="tips">';
    html += '<h5>📋 Measurement Tips:</h5>';
    html += '<ul>';
    html += '<li>Report final uncertainty to 1-2 significant figures</li>';
    html += '<li>Round final result to match uncertainty precision</li>';
    html += '<li>Check if systematic errors are considered</li>';
    html += '<li>Verify that errors are truly uncorrelated</li>';
    html += '</ul>';
    html += '</div>';
