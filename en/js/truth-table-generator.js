document.addEventListener("DOMContentLoaded", function () {
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (!expression) {
        result.innerHTML = '<div class="error">Please enter a logical expression.</div>';
        return;
      }
      ...
      try {
        ...
        result.innerHTML = `<div class="error">Error in expression: ${error.message}</div>`;
      }
    });
  }
  ...
  function generateTruthTable(expression) {
    ...
    if (variables.length > 4) {
      throw new Error("Too many variables (maximum 4: A, B, C, D)");
    }
  ...
  function displayTruthTable(variables, results, expression) {
    ...
    html += `<h3>📊 Truth Table for: <code>${expression}</code></h3>`;
    ...
    html += '<h4>📈 Analysis:</h4>';
    html += `<p>🟢 True values: <strong>${trueCount} of ${totalCount}</strong> (${percentage}%)</p>`;
    html += `<p>🔴 False values: <strong>${totalCount - trueCount} of ${totalCount}</strong> (${(100 - percentage).toFixed(1)}%)</p>`;
    
    if (trueCount === totalCount) {
      html += '<p>✅ This is a <strong>tautology</strong> - the expression is always true!</p>';
    } else if (trueCount === 0) {
      html += '<p>❌ This is a <strong>contradiction</strong> - the expression is always false!</p>';
    } else {
      html += '<p>🔀 This is a <strong>contingent</strong> expression - depends on variable values.</p>';
    }
    html += '</div>';
