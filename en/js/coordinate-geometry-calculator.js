// Define the main calculation function globally so it's available for onclick handlers
window.calculateCoordinateGeometry = function() {
  ...
  if (isNaN(x1) || isNaN(y1) || isNaN(x2) || isNaN(y2)) {
    if (result) result.innerHTML = '<div class="error">⚠️ Please enter valid values for all coordinates</div>';
    return;
  }
  ...
  const activeCalcBtn = document.querySelector('.calc-btn.active');
  const activeCalc = activeCalcBtn ? activeCalcBtn.dataset.calc : 'distance';
  
  let details = `
    <div class="calculation-details">
      <h4>📊 Calculation Results</h4>
      <div class="points-info">
        <span>Point A: (${x1}, ${y1})</span>
        <span>Point B: (${x2}, ${y2})</span>
      </div>
      <hr>
  `;
    ...
    case 'distance':
      details += `
        <div class="distance-result highlight">
          📏 <strong>Distance between points:</strong> ${distance.toFixed(4)}
    ...
    case 'midpoint':
      details += `
        <div class="midpoint-result highlight">
          📍 <strong>Midpoint of segment:</strong> (${midX.toFixed(4)}, ${midY.toFixed(4)})
    ...
    case 'slope':
      details += `
        <div class="slope-result highlight">
          📐 <strong>Line slope:</strong> ${slopeText}
          <div class="slope-details">
            <span>Slope coefficient: ${slopeText}</span>
            <span>Angle of inclination: ${angleText}</span>
          </div>
          ${slope !== null ? 
            `<small>m = (${y2} - ${y1})/(${x2} - ${x1}) = ${(y2-y1).toFixed(2)}/${(x2-x1).toFixed(2)}</small>` : 
            '<small>Line is vertical: x₁ = x₂</small>'
          }
        </div>
      `;
      break;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Only continue if we have calculation buttons
  if (calcButtons.length === 0) return;
  
  // The main calculation function is already defined globally above
  // Now we just need to set up event listeners and initial state
    // Get elements each time to ensure they're available
    const result = document.getElementById('coordinate-result');
    const canvas = document.getElementById('coordinate-canvas');
    const ctx = canvas ? canvas.getContext('2d') : null;
  ...
  calcButtons.forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      window.calculateCoordinateGeometry();
    });
  });
  ...
  inputs.forEach(input => {
    input.addEventListener('input', window.calculateCoordinateGeometry);
  });
