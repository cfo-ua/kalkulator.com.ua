document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener('submit', function (e) {
      ...
      try {
        ...
        if (activeTab === 'truth-table') {
          ...
          if (!values) {
            throw new Error("Please enter function values");
          }
        ...
        if (functionValues.length !== Math.pow(2, numVars)) {
          throw new Error(`Expected ${Math.pow(2, numVars)} values for ${numVars} variables`);
        }
  ...
  function displayKarnaughMap(kmap, groups, expression, numVars, variables) {
    ...
    html += `<h3>🗺️ Karnaugh Map for ${numVars} Variables</h3>`;
    ...
    html += '<h4>📈 Analysis:</h4>';
    html += `<p>🔢 Groups found: <strong>${groups.length}</strong></p>`;
    html += `<p>⚡ Minimized expression: <code>${expression}</code></p>`;
    
    const onesCount = kmap.flat().filter(x => x === 1).length;
    const totalCells = Math.pow(2, numVars);
    html += `<p>📊 Coverage: <strong>${onesCount}/${totalCells}</strong> cells</p>`;
    
    if (onesCount === totalCells) {
      html += '<p>✅ Function is always <strong>true</strong> (tautology)</p>';
    } else if (onesCount === 0) {
      html += '<p>❌ Function is always <strong>false</strong> (contradiction)</p>';
    } else {
      const coverage = ((onesCount / totalCells) * 100).toFixed(1);
      html += `<p>📈 Function is true for <strong>${coverage}%</strong> of input combinations</p>`;
    }
  ...
  function generateKMapHTML(kmap, groups, numVars, variables) {
    ...
    if (numVars === 2) {
      ...
      html += '<tr><th rowspan="2">AB\\CD</th><th>C\'D\'</th><th>C\'D</th><th>CD</th><th>CD\'</th></tr>';
      html += '<tr><th>00</th><th>01</th><th>11</th><th>10</th></tr>';
      
      const rowLabels = ['A\'B\' (00)', 'A\'B (01)', 'AB (11)', 'AB\' (10)'];
      for (let i = 0; i < 4; i++) {
        html += `<tr><th>${rowLabels[i]}</th>`;
        for (let j = 0; j < 4; j++) {
          html += `<td class="kmap-cell" data-row="${i}" data-col="${j}">${kmap[i][j]}</td>`;
        }
        html += '</tr>';
      }
    }
