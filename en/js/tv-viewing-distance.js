document.addEventListener("DOMContentLoaded", function () {
  ...
  const texts = {
    units: {
      metric: { distance: "m", small: "cm" },
      imperial: { distance: "ft", small: "in" }
    },
    standards: {
      "4k-optimal": "4K Optimal",
      "thx": "THX Standard", 
      "smpte": "SMPTE Standard",
      "gaming": "Gaming"
    },
    benefits: {
      "4k-optimal": "Maximum detail",
      "thx": "Cinematic experience",
      "smpte": "Comfortable viewing",
      "gaming": "Quick reaction"
    }
  };
  ...
  function calculateDistance() {
    ...
    if (!tvSize || tvSize < 20 || tvSize > 150) {
      alert("Please enter a valid TV size (20-150 inches)");
      return;
    }
  ...
  function createDistanceChart(distances, units) {
    ...
    const data = {
      labels: ['4K Optimal', 'THX', 'SMPTE', 'Gaming'],
      datasets: [{
        label: `Distance (${distances.recommended.unit})`,
        data: [
    ...
    const config = {
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text: '📊 Viewing Distance Comparison',
            font: { size: 16 }
          },
          legend: {
            display: false
        ...
        scales: {
          y: {
            ...
            title: {
              display: true,
              text: `Distance (${distances.recommended.unit})`
            }
          }
        }
      }
    ...
    const standards = [
      {
        ...
        benefits: "Best balance of detail and comfort"
      },
      {
        name: "THX Standard",
        distance: `${distances.thx.display} ${distances.thx.unit}`,
        benefits: "Cinematic experience, immersive viewing"
      },
      {
        name: "SMPTE Standard", 
        distance: `${distances.smpte.display} ${distances.smpte.unit}`,
        benefits: "Comfortable viewing without eye strain"
      },
      {
        name: "Gaming",
        distance: `${distances.gaming.display} ${distances.gaming.unit}`,
        benefits: "Quick reaction time, maximum focus"
      }
    ];
