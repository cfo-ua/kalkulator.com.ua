document.addEventListener("DOMContentLoaded", function () {
  ...
  function balanceEquation() {
    try {
      ...
      if (!equation) {
        throw new Error('Enter a chemical equation');
      }
  ...
  function parseEquation(equation) {
    ...
    if (parts.length !== 2) {
      throw new Error('Invalid equation format. Use → or =');
    }
  ...
  function displayResults(originalEquation, balanced, parsed) {
    let resultHTML = `<h4>⚖️ Balancing Results</h4>`;
    
    // Original equation
    resultHTML += `<div class="insight-card warning">
      <h6>📝 Original equation</h6>
    ...
    balanced.reactants.forEach(substance => {
      resultHTML += `<div class="insight-card info">
        ...
        <div class="small-text">reactant</div>
      </div>`;
    });
    ...
    balanced.products.forEach(substance => {
      resultHTML += `<div class="insight-card success">
        <h6>${substance.formula}</h6>
        <div class="big-number">${substance.coefficient}</div>
        <div class="small-text">product</div>
      </div>`;
    });
  ...
  function verifyBalance(balanced, elements) {
    ...
    verification += '<tr><th>Element</th><th>Reactants</th><th>Products</th><th>Balance</th></tr>';
    ...
    verification += `<div class="balance-summary ${isBalanced ? 'success' : 'error'}">
      ${isBalanced ? '✅ Equation balanced correctly!' : '❌ Equation is not balanced!'}
    </div>`;
  ...
  function generateStepsExplanation(balanced, parsed) {
    ...
    steps += '<h5>📋 Step by step:</h5>';
    steps += '<ol class="steps-list">';
    
    steps += '<li><strong>Molecule analysis:</strong> Identified elements in each substance</li>';
    steps += '<li><strong>Equation setup:</strong> Created system of equations for each element</li>';
    steps += '<li><strong>Solution:</strong> Found smallest integer coefficients</li>';
    steps += '<li><strong>Verification:</strong> Confirmed mass balance for all elements</li>';
