document.addEventListener("DOMContentLoaded", function () {
  ...
  function validateTriangle(a, b, c, A, B, C) {
    ...
    if (a && b && c) {
      if (a + b <= c || a + c <= b || b + c <= a) {
        return "Invalid sides: triangle inequality not satisfied";
      }
    }
    ...
    if (angleCount >= 2 && Math.abs(angleSum + (C || B || A) - 180) > 0.01) {
      if (angleCount === 3 && Math.abs(angleSum - 180) > 0.01) {
        return "Sum of angles must equal 180°";
      }
    }
  ...
  function solveTriangle(sideA, sideB, sideC, angleA, angleB, angleC) {
    ...
    if (knownSides + knownAngles < 3) {
      throw new Error("Need to know at least 3 parameters");
    }
    ...
    // Case 2: SSA - two sides and one angle (potentially ambiguous)
      ...
      if (a && b && A) {
        ...
        if (sinB > 1) {
          throw new Error("No solution exists: sinB > 1");
        }
      ...
      if (a && c && A) {
        ...
        if (sinC > 1) throw new Error("No solution exists");
    ...
    form.addEventListener('submit', function (e) {
      ...
      try {
        ...
        let resultHTML = `
          <div class="insight-cards">
            ...
            <div class="insight-card warning">
              ...
              <div class="result-value">Area = ${area.toFixed(3)}</div>
              <div class="result-value">Perimeter = ${perimeter.toFixed(3)}</div>
              <div class="result-value">Circumradius = ${circumradius.toFixed(3)}</div>
            </div>
          </div>
        `;
        ...
        if (solution.ambiguous) {
          resultHTML += `
            <div class="insight-card warning">
              <h6>⚠️ Ambiguous Case</h6>
              <p>Two possible solutions exist for this triangle!</p>
            </div>
          `;
        }
