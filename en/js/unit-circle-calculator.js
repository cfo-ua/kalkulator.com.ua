document.addEventListener("DOMContentLoaded", function () {
  ...
  function generateQuestion(type, angle) {
    ...
    if (type === 'coordinates' || (type === 'mixed' && Math.random() < 0.33)) {
      return {
        type: 'coordinates',
        question: `What are the coordinates of the point on the unit circle for angle ${angle}°?`,
        angle: angle,
      ...
      return {
        type: 'angle',
        question: `Which angle corresponds to coordinates (${x.toFixed(2)}, ${y.toFixed(2)})?`,
        coordinates: [x, y],
        correct: `${angle}°`,
  ...
  function showQuestion() {
    ...
    questionEl.innerHTML = `
      <h5>Question ${currentQuiz.currentQuestion + 1}/${currentQuiz.questions.length}</h5>
    ...
    resultsEl.innerHTML = `
      <div class="quiz-final-results">
        ...
        <div class="results-summary">
          ...
          <div class="time-display">Time: ${elapsed}s</div>
  ...
  function getPerformanceMessage(percentage) {
    if (percentage >= 90) return '🏆 Excellent! You know the unit circle well!';
    if (percentage >= 70) return '👍 Good! A bit more practice and you\'ll be perfect!';
    if (percentage >= 50) return '📚 Not bad, but review the material';
    return '💪 Need more practice. Don\'t give up!';
  }
  
  function updateQuizScore() {
    document.getElementById('quiz-score').textContent = 
      `Score: ${currentQuiz.correctAnswers}/${currentQuiz.currentQuestion}`;
  }
  ...
  function startQuizTimer() {
    ...
    quizTimer = setInterval(() => {
      seconds++;
      document.getElementById('quiz-timer').textContent = `Time: ${seconds}s`;
    }, 1000);
  }
  ...
  function showPracticeTopic(topic, contentEl) {
    const topics = {
      quadrants: {
        title: '🏠 Unit Circle Quadrants',
        content: `
          <div class="practice-topic">
            <p>The unit circle is divided into four quadrants:</p>
            <ul>
              <li><strong>I quadrant (0° - 90°):</strong> sin > 0, cos > 0</li>
              <li><strong>II quadrant (90° - 180°):</strong> sin > 0, cos < 0</li>
              <li><strong>III quadrant (180° - 270°):</strong> sin < 0, cos < 0</li>
              <li><strong>IV quadrant (270° - 360°):</strong> sin < 0, cos > 0</li>
            </ul>
            <p>Remember: <em>"All Students Take Calculus"</em> (all positive, sin, tan, cos)</p>
          </div>
        `
      },
      'special-angles': {
      ...
      signs: {
        title: '➕➖ Function Signs',
        content: `
          <div class="practice-topic">
            <p>Signs of trigonometric functions in quadrants:</p>
      ...
      symmetry: {
        title: '🔄 Unit Circle Symmetry',
        content: `
          <div class="practice-topic">
            <p>Useful symmetry relationships:</p>
            <ul>
              <li><strong>sin(-θ) = -sin(θ)</strong> (odd function)</li>
              <li><strong>cos(-θ) = cos(θ)</strong> (even function)</li>
    ...
    const topicData = topics[topic] || topics.quadrants;
    contentEl.innerHTML = `
