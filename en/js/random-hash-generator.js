document.addEventListener('DOMContentLoaded', function() {
    ...
    let history = JSON.parse(localStorage.getItem('randomHashHistory_en') || '[]');
    let statistics = JSON.parse(localStorage.getItem('randomHashStats_en') || '{}');
    let currentHashes = [];
    ...
    function validateInputs() {
        ...
        if (!isValid) {
            showError('Number of hashes must be between 1 and 100');
        } else {
            hideError();
        }
    ...
    function generateSingleHash(algorithm) {
        ...
        // Use crypto.getRandomValues for quality randomization
        ...
        generatedHashes.innerHTML = hashes.map((hashObj, index) => `
            <div class="hash-item" data-hash="${hashObj.value}">
                <div class="hash-meta">
                    ...
                    Hash ${index + 1} • ${hashObj.length} characters
                    ${includeTimestampsCheckbox.checked ? ` • ${new Date(hashObj.timestamp).toLocaleString('en-US')}` : ''}
                </div>
                <div class="hash-value">${hashObj.value}</div>
                <div class="hash-actions">
                    <button class="hash-btn" onclick="copyHash('${hashObj.value}')">📋 Copy</button>
                    <button class="hash-btn" onclick="validateSingleHash('${hashObj.value}', '${hashObj.algorithm}')">✅ Validate</button>
                    <button class="hash-btn" onclick="analyzeHash('${hashObj.value}', '${hashObj.algorithm}')">🔍 Analyze</button>
                </div>
                <div class="copy-notification">Copied!</div>
            </div>
        `).join('');
        
        generationInfo.textContent = `Generated ${hashes.length} hashes using ${hashAlgorithmSelect.value.toUpperCase()} algorithm. ` +
            `Average length: ${Math.round(hashes.reduce((sum, h) => sum + h.length, 0) / hashes.length)} characters.`;
        ...
        validationResults.innerHTML = results.map(result => `
            <div class="validation-item ${result.isValid ? '' : 'validation-error'}">
                <div style="font-weight: bold; margin-bottom: 0.25rem;">
                    ${result.isValid ? '✅' : '❌'} Hash ${result.index} (${result.algorithm.toUpperCase()})
                </div>
                <div style="font-family: 'Courier New', monospace; font-size: 0.9rem; margin-bottom: 0.5rem;">
                    ${result.value}
                </div>
                <div style="font-size: 0.8rem; color: #666;">
                    Length: ${result.actualLength}/${result.expectedLength} characters
    ...
    function addToHistory(hashes) {
        const entry = {
            ...
            hashes: hashes.slice(0, 3) // Save only first 3 to save space
        };
        
        history.unshift(entry);
        history = history.slice(0, 50); // Keep last 50 entries
        localStorage.setItem('randomHashHistory_en', JSON.stringify(history));
        updateHistoryDisplay();
    }
    ...
    function updateHistoryDisplay() {
        ...
        historyList.innerHTML = history.slice(0, 10).map((entry, index) => `
            <div class="validation-item">
                <div style="font-weight: bold; margin-bottom: 0.25rem;">
                    ${entry.timestamp} - ${entry.algorithm.toUpperCase()} (${entry.count} hashes)
                </div>
                <div style="font-family: 'Courier New', monospace; font-size: 0.85rem; margin-bottom: 0.5rem;">
                    ${entry.hashes[0].value}${entry.count > 1 ? ` (+${entry.count - 1} more)` : ''}
                </div>
                <div style="font-size: 0.8rem; color: #666;">
                    Format: ${entry.format}
                </div>
    ...
    function updateStatistics(algorithm, count) {
        ...
        localStorage.setItem('randomHashStats_en', JSON.stringify(statistics));
        updateStatisticsDisplay();
    }
    ...
    function updateStatisticsDisplay() {
        ...
        statisticsChart.innerHTML = `
            <div class="statistics-card">
                ...
                <div>Total sessions: <strong>${totalSessions}</strong></div>
                <div>Total hashes: <strong>${totalHashes}</strong></div>
            </div>
            ${Object.entries(statistics).map(([algorithm, stat]) => `
                <div class="statistics-card">
                    <h6>${algorithm.toUpperCase()}</h6>
                    <div>Sessions: ${stat.count}</div>
                    <div>Hashes: ${stat.totalGenerated}</div>
                    <div>Average: ${Math.round(stat.totalGenerated / stat.count)}</div>
                </div>
            `).join('')}
        `;
    ...
    function clearHistory() {
        if (confirm('Are you sure you want to clear all history and statistics?')) {
            history = [];
            statistics = {};
            localStorage.removeItem('randomHashHistory_en');
            localStorage.removeItem('randomHashStats_en');
            updateHistoryDisplay();
            updateStatisticsDisplay();
        }
    }
    ...
    window.validateSingleHash = function(hash, algorithm) {
        ...
        alert(isValid ? 'Hash has correct format!' : 'Hash has incorrect format!');
    };
    ...
    window.analyzeHash = function(hash, algorithm) {
        ...
        alert(`Hash Analysis for ${algorithm.toUpperCase()}:\n` +
              `Length: ${cleanHash.length} characters\n` +
              `Unique characters: ${uniqueChars}/16\n` +
              `Entropy: ${entropy.toFixed(2)} bits\n` +
              `Most frequent character: ${Object.entries(charFrequency).sort((a,b) => b[1] - a[1])[0][0]} (${Object.entries(charFrequency).sort((a,b) => b[1] - a[1])[0][1]} times)`);
    };
