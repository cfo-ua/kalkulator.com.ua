document.addEventListener("DOMContentLoaded", function () {
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (isNaN(P)) {
        result.innerHTML = `<b>P =</b> ${(n * R * T / V).toFixed(4)} atm`;
      } else if (isNaN(V)) {
        result.innerHTML = `<b>V =</b> ${(n * R * T / P).toFixed(4)} L`;
      } else if (isNaN(n)) {
        result.innerHTML = `<b>n =</b> ${(P * V / (R * T)).toFixed(4)} mol`;
      } else if (isNaN(T)) {
        result.innerHTML = `<b>T =</b> ${(P * V / (n * R)).toFixed(4)} K`;
      }
    });
  }
});