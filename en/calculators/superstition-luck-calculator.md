...
<form id="superstition-form" autocomplete="off">
  <div class="form-section">
    <h3>📅 Date & Timing Factors</h3>
    <label>
      What day is today?
      <select id="day-type" required>
        <option value="">Select day type...</option>
        <option value="friday-13th">Friday the 13th 😱</option>
        <option value="friday">Regular Friday</option>
        ...
        <option value="birthday">Your birthday or anniversary</option>
      </select>
    </label>
    
    <label>
      Current moon phase (approximate):
    ...
    <label>
      ...
      <select id="time-of-day" required>
        ...
        <option value="late-night">Late night (12-5 AM)</option>
      </select>
    </label>
  </div>

  <div class="form-section">
    <h3>🔢 Personal Number Influences</h3>
    <label>
      Your lucky number (1-99):
      <input type="number" id="lucky-number" min="1" max="99" placeholder="Enter your lucky number">
    </label>
    
    <label>
      Today's date adds up to (day + month):
      <select id="date-sum" required>
        <option value="">Calculate and select...</option>
        <option value="7">7 (Very lucky!)</option>
        <option value="8">8 (Lucky in Chinese culture)</option>
        <option value="9">9 (Lucky completion number)</option>
        <option value="11">11 (Master number)</option>
        <option value="13">13 (Unlucky in Western culture)</option>
        <option value="21">21 (Lucky blackjack number)</option>
        <option value="other-lucky">Other number I consider lucky</option>
    ...
    <div class="checkbox-group">
      <label>
        <input type="checkbox" id="repeated-numbers"> Seeing repeated numbers lately (11:11, 2:22, etc.)
      </label>
    </div>
  </div>

  <div class="form-section">
    <h3>🐾 Recent Animal & Nature Encounters</h3>
    <p style="font-size: 0.9rem; color: #666;">Select any you've encountered in the past week:</p>
    
    <div class="checkbox-group">
      <label>
        <input type="checkbox" id="black-cat"> 🐈‍⬛ Black cat crossed your path
      </label>
      <label>
        <input type="checkbox" id="ladybug"> 🐞 Ladybug landed on you
      </label>
      <label>
        <input type="checkbox" id="butterfly"> 🦋 Butterfly near you
      </label>
      <label>
        <input type="checkbox" id="bird-window"> 🐦 Bird hit your window
      </label>
      <label>
        <input type="checkbox" id="spider-morning"> 🕷️ Small spider in the morning
      </label>
      <label>
        <input type="checkbox" id="robin"> 🐦 Robin (first bird of spring)
      </label>
    </div>
    
    <div class="checkbox-group">
      <label>
        <input type="checkbox" id="rainbow"> 🌈 Saw a rainbow
      </label>
      <label>
        <input type="checkbox" id="shooting-star"> ⭐ Saw a shooting star
      </label>
      <label>
        <input type="checkbox" id="four-leaf-clover"> 🍀 Found a four-leaf clover
      </label>
      <label>
        <input type="checkbox" id="penny-heads"> 🪙 Found a heads-up penny
      </label>
    </div>
  </div>

  <div class="form-section">
    <h3>🏠 Recent Events & Actions</h3>
    <p style="font-size: 0.9rem; color: #666;">Things that happened recently:</p>
    
    <div class="checkbox-group">
      <label>
        <input type="checkbox" id="broken-mirror"> 🪞 Broke a mirror
      </label>
      <label>
        <input type="checkbox" id="walked-under-ladder"> 🪜 Walked under a ladder
      </label>
      <label>
        <input type="checkbox" id="spilled-salt"> 🧂 Spilled salt
      </label>
      <label>
        <input type="checkbox" id="umbrella-indoors"> ☂️ Opened umbrella indoors
      </label>
      <label>
        <input type="checkbox" id="shoes-on-bed"> 👠 Put shoes on bed
      </label>
      <label>
        <input type="checkbox" id="hat-on-bed"> 👒 Put hat on bed
      </label>
    </div>
    
    <div class="checkbox-group">
      <label>
        <input type="checkbox" id="wished-on-star"> ⭐ Made a wish on a star
      </label>
      <label>
        <input type="checkbox" id="knocked-on-wood"> 🪵 Knocked on wood
      </label>
      <label>
        <input type="checkbox" id="crossed-fingers"> 🤞 Crossed fingers for luck
      </label>
      <label>
        <input type="checkbox" id="threw-salt"> 🧂 Threw salt over shoulder after spilling
      </label>
    </div>
  </div>

  <div class="form-section">
    <h3>💭 Personal Beliefs & Attitude</h3>
    <label>
      How much do you believe in superstitions?
      <select id="belief-level" required>
        <option value="">Select belief level...</option>
        <option value="strong">Strong believer - They definitely affect luck</option>
        <option value="moderate">Moderate believer - Some truth to them</option>
        <option value="casual">Casual - Fun but not serious</option>
        <option value="skeptical">Skeptical - Don't really believe</option>
        <option value="none">Non-believer - Pure coincidence</option>
      </select>
    </label>
    
    <label>
      Your general mood today:
      <select id="mood" required>
        <option value="">Select mood...</option>
        <option value="excellent">Excellent - Feeling amazing</option>
        <option value="good">Good - Positive and happy</option>
        <option value="neutral">Neutral - Just okay</option>
        <option value="low">Low - Not feeling great</option>
        <option value="bad">Bad - Having a rough day</option>
      </select>
    </label>
    
    <label>
      Do you carry any lucky charms?
      <select id="lucky-charms" required>
        <option value="">Select charm status...</option>
        <option value="multiple">Multiple lucky charms/items</option>
        <option value="one-special">One special lucky item</option>
        <option value="occasional">Occasionally carry something lucky</option>
        <option value="none">Don't carry lucky charms</option>
      </select>
    </label>
    
    <div class="checkbox-group">
      <label>
        <input type="checkbox" id="positive-thinking"> Actively practicing positive thinking
      </label>
      <label>
        <input type="checkbox" id="grateful-mindset"> Feeling grateful today
      </label>
      <label>
        <input type="checkbox" id="expecting-good"> Expecting good things to happen
      </label>
    </div>
  </div>

  <button type="submit">🍀 Calculate My Luck Score</button>
</form>

<div id="superstition-result" class="result"></div>